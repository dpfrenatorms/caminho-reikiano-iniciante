...
| F7 Montagem do curso (Nutror) + **upsell R$197** + trilhas de e-mail | ⏳ após F6 — **upsell construído e com o vídeo embutido**. Página: `landing-page/upsell.html` (+ `landing-page/assets/`). Copy canônica: `curso/upsell/pagina-upsell.md`. Mecanismo: `curso/upsell/metodo-maos-seguras.md`. Roteiro gravado: `curso/narracao/UPSELL-video-renato.txt`. ⛔ **Não publicar a página antes do curso existir** — go-live só no **Sprint 5**, depois de gravar as 20 aulas (Sprints 1–3) e montar o produto (Sprint 4). Lá também se configura o upsell one-click na Eduzz (URLs de aceite/recusa, entrega do e-book se recusar, rastreamento) e **se retira a lista de espera da LP**. **Decisão 09/07: manter a lista de espera até a gravação e o lançamento do curso.** |
