...
| F7 Montagem do curso (Nutror) + **upsell R$197** + trilhas de e-mail | ⏳ após F6 — **upsell construído e com o vídeo embutido**. Página: `landing-page/upsell.html` (+ `landing-page/assets/`). Copy canônica: `curso/upsell/pagina-upsell.md`. Mecanismo: `curso/upsell/metodo-maos-seguras.md`. Roteiro gravado: `curso/narracao/UPSELL-video-renato.txt`. **Falta:** publicar a página + configurar o upsell one-click na Eduzz (URLs de aceite/recusa, entrega do e-book se recusar, rastreamento). **Decisão 09/07: manter a lista de espera na LP inicial por ora** (curso ainda não gravado). |
